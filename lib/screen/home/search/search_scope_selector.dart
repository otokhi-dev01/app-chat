...
/// REPLACED: Custom interactive SearchScopeSelector with sliding indicator pill, drag gestures, and unit UI styling
...
class _SearchScopeSelectorState extends State<SearchScopeSelector> {
  ...
  // ADDED: Search scope filter items (Chats, Contacts, All) paired with Cupertino icons
  List<SearchScopeFilterItem> get filterItems {
    return <SearchScopeFilterItem>[
      SearchScopeFilterItem(
  ...
  int get selectedIndex {
    int index = filterItems.indexWhere(
          (SearchScopeFilterItem item) => item.type == widget.selectedScope,
    );
  ...
  List<Widget> _buildButtons({required double itemWidth}) {
    ...
    for (int index = 0; index < filterItems.length; index++) {
      SearchScopeFilterItem item = filterItems[index];

      widgets.add(
        SizedBox(
          width: itemWidth,
          height: itemHeight,
          child: _SearchScopeFilterButton(
            title: item.title,
            icon: item.icon,
...
class _SearchScopeFilterButton extends StatelessWidget {
  final String title;
  final IconData icon;
  final bool selected;
  final VoidCallback onTap;

  const _SearchScopeFilterButton({
    required this.title,
    required this.icon,
